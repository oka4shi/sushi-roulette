    ...
    if category_name not in ['はまっこセット','地域限定','お持ち帰り']:
        ...
        for item in items:
            ...
            if text1:
                ...
                text1 = ''.join([text for text in text1.get_text(',').split(',') if '円' not in text])

                text2 = item.select_one('div.men-products-item__small')
                if text2:
                    text2 = ''.join([text for text in text2.get_text(',').split(',') if re.match(r'（.*）',text)])
                    name = f"{text1}{text2}"
                else:
                    name = text1

                img_url = item.select_one('div.men-products-item__thumb > img')['data-src']
